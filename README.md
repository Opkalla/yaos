...
- **Spoke access is set per room by the hub** — one of three tiers: **Read-only** (spokes see changes live but write nothing back), **Edit shared notes** (the default — spokes edit the hub's notes, but creating, renaming, and deleting stay hub-only), or **Full access** (spokes may also add, rename, and delete notes in the room). Keeping structure hub-only by default means folder structure stays authoritative and avoids conflicting structural edits arriving from several vaults at once. When a spoke's change is blocked, Lodestone tells them why instead of silently dropping it. Enforced by the plugin on each spoke, not by the server — see the caveat below.
...
**I created a file in a shared room folder on a spoke, but it never showed up on the hub or other spokes.** Expected on the default access tier — see [Rooms](#rooms--sharing-folders-across-vaults) above. Adding, renaming, and deleting notes in a room is hub-only unless the hub sets that room to **Full access**. The file stays on that spoke's vault; Lodestone shows a notice explaining that it won't propagate. Ask the room's host to either create the note themselves or raise the room's access tier.

**Can spokes edit shared files in a room?** On the default tier, yes — real-time content editing and live cursors work in both directions between hub and spokes, and only *structural* changes (create/rename/move/delete) are host-only. A hub can set a room to **Read-only** instead, which makes the notes non-editable on every spoke, or to **Full access**, which lets spokes make structural changes too.

**Are access tiers a security boundary?** No. They are enforced by the Lodestone plugin on each spoke, not by the server, so a modified client could ignore them — the same limitation that has always applied to the host-only structure rule. Treat tiers as a shared agreement between vaults you trust, and don't put anything in a room that would be a problem if a member wrote to it.
